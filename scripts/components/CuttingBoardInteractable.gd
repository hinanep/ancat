...
## 初始化烹饪预览类型。
func _ready() -> void:
	cookPreviewFoodType = FoodConfig.FoodType.SASHIMI
	super._ready()
...
func _on_item_valid(player: Node, item: Node, anchorController: Node) -> void:
	...
	if item.is_in_group(plateGroupName):
		...
		_agent_debug_emit(
			'H5',
			'CuttingBoardInteractable.gd:_on_item_valid',
			'served food to plate',
			{'servedFoodType': FoodConfig.FoodType.SASHIMI, 'plateName': item.name}
		)
		#endregion
