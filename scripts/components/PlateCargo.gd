...
## 跨层摔碎阈值（像素）。
@export var breakDropHeightPx: float = 220.0
...
const FISH_SCENE: PackedScene = preload('res://scenes/props/FishEntity.tscn')
...
func _physics_process(delta: float) -> void:
	...
	if global_position.y < _dropStartY + threshold:
		return
	_break_plate()
